...
Upload.loadAssets = async function () {
    while ($(".upload-media-asset-loading").length) {
        let ids = $(".upload-media-asset-loading")
            .map((i, el) => $(el).attr("data-id"))
            .toArray()
            .join(",");
        let size = $(".upload-media-asset-gallery").attr("data-size");
        $.get("/upload_media_assets.js", {
            search: { status: "active failed", id: ids },
            size: size,
        });
        await delay(250);
    }
...
Upload.initialize_draggable_divider = function () {
    ...
    $(".upload-divider").on("drag:move", (event, moveEvent, drag) => {
        ...
        let minWidth = parseInt(
            $(".upload-container").css("--min-edit-container-width"),
        );
        let maxWidth = $(".upload-container").width() - minWidth;

        currentPanelWidth = clamp(
            initialPanelWidth - dragOffset,
            minWidth,
            maxWidth,
        );
